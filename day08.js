...
function processInput(lines) {
  ...
  for (let line of lines) {
    ...
    if (eval(get(condReg) + comparator + valueString)) {
      ...
      registers.set(modReg, (op == "inc") ? (oldRegVal + amount) : (oldRegVal - amount));
    } 
  }
  return Array.from(registers.values()).reduce((acc, cur) => Math.max(acc, cur), -Infinity);
}
/*
...
*/
console.assert(processInput(testInput) == 1);
